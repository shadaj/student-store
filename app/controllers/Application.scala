...
class Application @Inject() (
  ...
  def scan = IsAuthenticated { mode => implicit request =>
    {
      ...
      Ok(
        views.html.index(
          purchasesToday.map(p => p.product.price * p.quantity).sum,
          purchasesToday,
          mode
        )
      )
  ...
  def processBarcode = IsAuthenticated { mode => implicit request =>
    scanForm
      .bindFromRequest()
      .fold(
        ...
        barcode => {
          ...
          Barcodes.findByBarcode(barcode, mode) match {
            ...
            case None =>
              Redirect(routes.Application.scan)
                .flashing("warning" -> "Barcode was not found")
          }
        }
      )
  }
  ...
  def switchMode = IsAuthenticated { oldMode => implicit request =>
    {
      switchModeForm
        .bindFromRequest()
        .fold(
          errors => Redirect(routes.Application.scan),
          newMode => Redirect(routes.Application.scan).withSession("mode" -> newMode)
        )
    }
  }

  def manageProducts = IsAuthenticated { mode => implicit request =>
  ...
  def createProduct = IsAuthenticated { mode => implicit request =>
    createProductForm
      .bindFromRequest()
      .fold(
  ...
  def deleteProduct(id: Long) = IsAuthenticated { mode => _ =>
    Product.delete(id, mode)
    Redirect(routes.Application.manageProducts)
  }

  def editProduct(id: Long) = IsAuthenticated { mode => _ =>
  ...
  def updateProduct(id: Long) = IsAuthenticated { mode => implicit request =>
    createProductForm
      .bindFromRequest()
      .fold(
        errors => Redirect(routes.Application.manageProducts),
  ...
  def manageBarcodes = IsAuthenticated { mode => _ =>
    Ok(
      views.html
        .manageBarcodes(Barcodes.findAll(mode), Product.findAll(mode), mode)
    )
  }

  val createBarcodeForm = Form(
    tuple("barcode" -> nonEmptyText, "product" -> nonEmptyText)
  )

  def createBarcode = IsAuthenticated { mode => implicit request =>
    createBarcodeForm
      .bindFromRequest()
      .fold(
  ...
  def deleteBarcode(barcode: String) = IsAuthenticated { mode => _ =>
    Barcodes.delete(barcode, mode)
    Redirect(routes.Application.manageBarcodes)
  }

  def managePurchases = IsAuthenticated { mode => _ =>
    Ok(
      views.html.managePurchases(
        purchases.findAllSorted(mode),
        Product.findAll(mode),
        mode
      )
    )
  }

  val addPurchaseForm = Form(
    tuple(
  ...
  def addPurchase = IsAuthenticated { mode => implicit request =>
    addPurchaseForm
      .bindFromRequest()
      .fold(
        ...
        formValues => {
          ...
          if (quantity == 0) {
            purchases.findForDateAndProduct(
              mode,
              date,
              Product.findByName(product, mode).get
            ) match {
              case Some(p) =>
                purchases.delete(
                  p.date,
                  Product.findByName(product, mode).get.id,
                  mode
                )
              case None =>
            }
  ...
  def export = IsAuthenticated { mode => _ =>
    Ok(
      views.html.export(
        purchases.findAll(mode).map(_.date.toString("MM/YYYY")).distinct,
        mode
      )
    )
  }
  ...
  def getExport = IsAuthenticated { mode => implicit request =>
    val moneyFormat = "%.2f"
    getExportForm
      .bindFromRequest()
      .fold(
        ...
        date => {
          ...
          purchases
            .findAll(mode)
            .filter(p =>
              p.date.getMonthOfYear() == date.getMonthOfYear() && p.date
                .getYear() == p.date.getYear()
            )
            .groupBy(_.date)
            .toSeq
            .sortBy(_._1)
            .foreach {
              case (date, found) =>
                writer.writeRow(
                  List(
                    s"Total for day ${date.monthOfYear.get}/${date.dayOfMonth.get}/${date.year.get}",
                    "Item",
                    "Price",
                    "Unit Sold",
                    "Money Earned",
                  ...
                  case Purchase(_, product, quantity) =>
                    ...
                    writer.writeRow(
                      List(
                        ...
                        "$" + product.price,
                        quantity,
                        "$" + moneyFormat.format(moneyEarned),
                        "$" + product.bought,
                        "$" + moneyFormat.format(individualProfit),
                ...
                writer.writeRow(
                  List(
                    ...
                    "",
                    "",
                    "Total Earned: $" + moneyFormat.format(total)
                  )
                )
          ...
          writer.writeRow(
            Seq("Total for all days: $" + moneyFormat.format(overallTotal))
          )
          writer.close()
          Ok.sendFile(file)
        }
  ...
  def authenticate = Action { implicit request =>
    loginForm
      ...
      .fold(
        ...
        user =>
          if (user._1 == sys.env("AUTH_USER") && user._2 == sys.env("AUTH_PASS")) {
            Redirect(routes.Application.scan)
              .withSession("mode" -> "Student Store")
          } else {
  ...
  def IsAuthenticated(f: => String => Request[AnyContent] => Result) =
    Security.Authenticated(mode, onUnauthorized)(mode => Action(request => f(mode)(request)))
}
